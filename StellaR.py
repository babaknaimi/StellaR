...
def main():
    if len(sys.argv) <  3:
        ...
        print ("\t -- outname is the name of R project where the main R script (outname.R) as well as data files and other functions are stored\n")
        sys.exit()
    ...
    for fl in list(flows.keys()):
        ...
        for i in range(0, len(conv_temp)):
            ...
            if ((conv_temp[i] not in restricted_words) and (not is_number(conv_temp[i]))):
                if (conv_temp[i] not in list(convertors.keys()) and conv_temp[i] not in list(models.keys()) and conv_temp[i] not in list(flows.keys())):
    ...
    for conv in list(convertors.keys()):
        ...
        if ('GRAPH' in txtline): convertors[conv].isData=True
        elif (is_number(txtline)): convertors[conv].value=float(txtline)
        else:
            ...
            conv_temp = re.split('=|\*|>|<|,|\+|\^|\-|\)|\(|[{]|[}]|/|\s+|%%', convertors[conv].Eq)
            restricted_words = ['','and','if','or','If','IF','AND','THEN','then','ELSE','else','OR'] + supported_func
            for i in range(0, len(conv_temp)):
                if ((conv_temp[i] not in restricted_words) and (not is_number(conv_temp[i]))):
                    if (conv_temp[i] not in list(convertors.keys()) and conv_temp[i] not in list(newconv.keys()) and conv_temp[i] not in list(models.keys()) and conv_temp[i] not in list(flows.keys())):
                        newconv[conv_temp[i]]=convertor(conv_temp[i])
                    if (conv_temp[i] not in convertors[conv].in_convertors): convertors[conv].in_convertors.append(conv_temp[i])
    ...
    while (rr):
        ...
        for add_conv in list(newconv.keys()):
            ...
            if ('GRAPH' in txtline): convertors[add_conv].isData=True
            elif (is_number(txtline)): convertors[add_conv].value=float(txtline)
            else:
                ...
                for i in range(0, len(conv_temp)):
                    if ((conv_temp[i].strip() not in restricted_words) and (not is_number(conv_temp[i].strip()))):
                        if (conv_temp[i].strip() not in list(convertors.keys()) and conv_temp[i].strip() not in list(models.keys()) and conv_temp[i].strip() not in list(flows.keys())):
        ...
        if len(list(newconv2.keys()))>0: newconv=newconv2
        else: rr=False
    ...
    if (len(list(additional_lines.keys())) > 0):
        ...
        for i in range(0, len(list(additional_lines.keys()))):
            if ('\r' in additional_lines[list(additional_lines.keys())[i]]): additional_lines[list(additional_lines.keys())[i]] = additional_lines[list(additional_lines.keys())[i]].replace('\r', " ")
